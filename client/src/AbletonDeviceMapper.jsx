...
export default function AbletonDeviceMapper() {
  ...
  useEffect(() => {
    ...
    const handler = (event) => {
      ...
      const source = { id, endpointName: input.name || 'Unnamed MIDI input', userChannel: frameworkChannel + 1, frameworkChannel, data1, lastValue: data2, controlKind: 'knob', label: `CC ${data1}` }
  ...
  const autoBuild = () => {
    ...
    for (const layoutId of getBestLayoutIds(selectedDevice)) {
  ...
  const chooseParameter = (mapping, name) => {
    ...
    updateMapping(mapping.id, parameter ? { targetParameterName: parameter.name, parameterAliases: [parameter.name], parameterIndex: parameter.parameterIndex, liveIndex: parameter.liveIndex, parameterSection: parameter.section || 'Unclassified', parameterRisk: parameter.risk || 'unknown' } : { targetParameterName: '', parameterAliases: [], parameterIndex: null, liveIndex: null, parameterSection: 'Unassigned' })
  }
  const assignSource = (mappingId, id) => updateMapping(mappingId, { source: controls.find((control) => control.id === id) || null })
...
function GroupedMappingMatrix({mappings,stack,controls,device,warnings,learningMappingId,updateMapping,chooseParameter,assignSource,learn,remove}) {
