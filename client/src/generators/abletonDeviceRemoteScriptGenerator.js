const normalizeIdentifier = (value) => {
  const normalized = String(value || '')
    ...
    .replace(/^_+|_+$/g, '')
  const safe = normalized || 'Ableton_Device'
  return /^\d/.test(safe) ? `Device_${safe}` : safe
}
...
export const createAbletonDeviceScriptSlug = (deviceName) => `${normalizeIdentifier(deviceName)}_Device_Remote`

export function createAbletonDeviceProfile({ device, mappings, scriptSlug = createAbletonDeviceScriptSlug(device.deviceName) }) {
  return {
    schemaVersion: '0.1',
...
export function generateAbletonDeviceRemoteScriptFiles({ device, mappings, scriptSlug = createAbletonDeviceScriptSlug(device.deviceName) }) {
  const profile = createAbletonDeviceProfile({ device, mappings, scriptSlug })
